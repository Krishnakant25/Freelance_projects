...
class Role(str, Enum):
    STAFF = "staff"   # read tickets, acknowledge, resolve
    ADMIN = "admin"   # everything staff can do, plus trigger alerts/escalations
...
class Principal:
    ...
    def has(self, role: Role) -> bool:
        # admin implies staff — avoids having to grant both on every admin key
        # and avoids the failure mode where an admin key mysteriously can't
        # read the queue it's administering.
        if Role.ADMIN.value in self.roles:
            return True
...
def _authenticate(x_api_key: Optional[str]) -> Principal:
    if not config.AUTH_ENABLED:
        ...
        logger.warning(
            "AUTH_ENABLED=false — staff endpoint served WITHOUT authentication. "
            "This must never be the case in a deployed environment."
        )
        return Principal(name="dev-bypass", roles=[Role.ADMIN.value])

    if not x_api_key:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Missing X-API-Key header. Staff endpoints require a key.",
...
def require_staff(x_api_key: Optional[str] = Header(default=None)) -> Principal:
    """FastAPI dependency for endpoints that read or mutate ticket data."""
    principal = _authenticate(x_api_key)
    if not principal.has(Role.STAFF):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="This key does not have the 'staff' role.",
        )
    return principal
