...
Usage:
    ...
    python scripts/manage_keys.py create --name "security-lead" --roles approver,admin
    python scripts/manage_keys.py list
    python scripts/manage_keys.py revoke --name "ci-runner"
"""
import argparse
import json
import sys
...
def main():
    parser = argparse.ArgumentParser(description="Manage secure-agent API keys")
    sub = parser.add_subparsers(dest="command", required=True)

    p_create = sub.add_parser("create")
    p_create.add_argument("--name", required=True)
    p_create.add_argument("--roles", default="operator", help=f"Comma-separated: {sorted(VALID_ROLES)}")
